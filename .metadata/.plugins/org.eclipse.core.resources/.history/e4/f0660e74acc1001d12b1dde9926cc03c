...
uint8_t Hum_byte1, Hum_byte2, Temp_byte1, Temp_byte2;
...
uint8_t DHT_Check_Reponse (void)
{
	uint8_t Response = 0;
	delay_us(40);
...
void Display_Temp (float Temp)
{
	...
	sprintf(str, "Tempe: %.2f ", Temp);
	lcd_send_string(&p_lcd, str);
	lcd_send_data(&p_lcd,'C');
	delay_us(5000);
}

void Display_Humid (float Hum)
{
	char str[20];
